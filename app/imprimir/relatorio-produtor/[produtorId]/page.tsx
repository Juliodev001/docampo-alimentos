...
type Fechamento = {
  ...
  status: string
  bruto: number; deducaoRateada: number; abatimentoVales: number; liquido: number
  valorPagoManual: boolean
}
type Produtor = {
  id: string; nome: string; codigo: string | null
  cpf: string | null; cnpj: string | null
  inscricaoEstadual: string | null; telefone: string | null; endereco: string | null
}
...
export default function RelatorioProdutor() {
  ...
  const [data, setData] = useState<{ produtor: Produtor; fechamentos: Fechamento[] } | null>(null)
  ...
  useEffect(() => {
    ...
    const qs = p.toString()
    fetch(`/api/relatorio-produtor/${produtorId}${qs ? `?${qs}` : ''}`)
      .then(r => r.json())
      .then(setData)
  }, [produtorId, idsParam, dataInicioParam, dataFimParam])
  ...
  async function handleWhatsApp() {
    ...
    try {
      ...
      a.href = url
      a.download = `extrato-produtor-${data?.produtor.nome.replace(/\s+/g, '-').toLowerCase() ?? 'produtor'}.jpg`
      a.click()
  ...
  const { produtor, fechamentos } = data
  const totalBruto     = fechamentos.reduce((s, f) => s + f.bruto, 0)
  const totalDescontos = fechamentos.reduce((s, f) => s + f.deducaoRateada + f.abatimentoVales, 0)
  const totalLiquido   = fechamentos.reduce((s, f) => s + f.liquido, 0)
  const totalPendente  = fechamentos.filter(f => f.status !== 'PAGO').reduce((s, f) => s + f.liquido, 0)
        ...
        <div style={{ width: 480, padding: '10px 8px', fontSize: 10, position: 'relative' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: NAVY }}>
              Extrato de Fechamentos do Produtor
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 11 }}>
            ...
            <span><strong>Outorgado:</strong> {produtor.nome}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 11 }}>
            <span><strong>CPF/CNPJ:</strong> {produtor.cpf ?? produtor.cnpj ?? '—'}</span>
            <span><strong>Telefone:</strong> {produtor.telefone ?? '—'}</span>
          </div>
          {produtor.endereco && (
            <div style={{ marginBottom: 4, fontSize: 11 }}>
              <strong>Endereço:</strong> {produtor.endereco}
            </div>
          )}
          <div style={{ marginBottom: 14, fontSize: 11 }}>
            <strong>Inscrição Estadual:</strong> {produtor.inscricaoEstadual ?? '—'}
          </div>
                ...
                <tbody>
                  {fechamentos.map(f => (
                    <tr key={f.id}>
                      ...
                      <td style={{ ...cell, textAlign: 'right' as const }}>{fmtN(f.bruto)}</td>
                      <td style={{ ...cell, textAlign: 'right' as const }}>{fmtN(f.deducaoRateada + f.abatimentoVales)}</td>
                      <td style={{ ...cell, textAlign: 'right' as const, fontWeight: 700 }}>{fmtN(f.liquido)}</td>
