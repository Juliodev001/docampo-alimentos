...
--
-- Mesmo conteúdo da migração 20260817000000_indices_relatorios — aqui na forma
-- que se aplica direto pelo psql, que é como este banco recebe mudança.
--
-- Por que faltavam: o Postgres cria índice sozinho só para a chave primária e
-- para colunas UNIQUE — chave estrangeira NÃO ganha índice automático. Sem
-- eles, "as colheitas deste produtor neste período" varre a tabela inteira e só
-- depois joga fora o que não serve.
--
-- Criar índice tranca a tabela para escrita enquanto roda. Com o volume deste
-- banco é questão de segundos, mas rode fora do horário de lançamento.
...
-- O planejador de consultas só passa a usar índice novo depois de reestimar a
-- tabela; ANALYZE faz isso na hora, em vez de esperar o autovacuum.
ANALYZE;
